...
public class VersionUtils {
    ...
    private static final String NEXFLY_VERSION_FILE = "nexfly-version.txt";
    
    static {
        try (InputStream in = VersionUtils.class.getClassLoader().getResourceAsStream(NEXFLY_VERSION_FILE)) {
            Properties props = new Properties();
            props.load(in);
            String val = props.getProperty("version");
            if (val != null && !VERSION_PLACEHOLDER.equals(val)) {
                version = val;
                clientVersion = "Nexfly:v" + VersionUtils.version;
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
